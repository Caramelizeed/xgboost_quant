...
export function HeatmapChart({ heatmap, onCellClick }) {
  ...
  const values = heatmap.matrix.flat().filter((v) => typeof v === 'number');
  ...
  return (
    <div className="min-w-0 rounded-xl border border-border bg-surface p-4 shadow-[0_0_50px_rgba(0,0,0,0.15)] overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <div className="inline-grid rounded-xl border border-[#2b2b2b]" style={{ gridTemplateColumns: `140px repeat(${heatmap.x_axis.length}, minmax(60px, 1fr))` }}>
          ...
          {heatmap.x_axis.map((x) => (
            <div key={x} className="border-b border-[#2b2b2b] bg-[#121212] px-3 py-3 text-right text-[11px] uppercase tracking-[0.3em] text-muted">
              {x}
            </div>
          ))}
          {heatmap.y_axis.map((y, rowIndex) => (
            <React.Fragment key={y}>
              ...
              {heatmap.matrix[rowIndex].map((value, colIndex) => (
                <button
                  ...
                  type="button"
                  onClick={() => onCellClick?.({ [heatmap.param_x]: heatmap.x_axis[colIndex], [heatmap.param_y]: heatmap.y_axis[rowIndex] })}
